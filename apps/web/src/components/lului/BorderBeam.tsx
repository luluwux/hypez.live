...
interface BorderBeamProps {
    className?: string;
    size?: number;        // Beam uzunluğu
    duration?: number;    // Tur süresi
    borderWidth?: number; // Çizgi kalınlığı
    anchor?: number;      // Başlangıç noktası (0-100)
    colorFrom?: string;
    colorTo?: string;
    delay?: number;
}
...
export const BorderBeam = ({
    ...
    return (
        <div
            ...
            className={cn(
                // Ana Container: Kartın sınırlarına tam oturur
                "pointer-events-none absolute inset-0 rounded-[inherit] [border:calc(var(--border-width)*1px)_solid_transparent]",

                // Maskeleme: Sadece border alanını görünür kılar, içini boşaltır
                "![mask-clip:padding-box,border-box] ![mask-composite:intersect] [mask:linear-gradient(transparent,transparent),linear-gradient(white,white)]",

                // Mobilde taşmaları önler
