# <p align="center"><img src="./image/HypezLogo.png" alt="Hypez.live Logo" width="160"></p>

# <p align="center">Hypez.live — Discord Server Discovery Platform</p>
Hypez is the easiest way to discover, grow, and promote your Discord server.

Browse thousands of servers by category, vote for your favorites, and hype the communities you love. If you own a server, add it to Hypez — our Discord bot handles the rest.

Our ranking system goes beyond simple vote counts. Community activity, voice channel usage, and hype scores combine to surface the most active servers.

Hypez is currently in beta — getting better every day.

## ✨ Image 

### Home Page
<details>
  <summary style="display: block;"></summary>
  <p align="center"><img src="./image/HypezHomepage.png" alt="Hypez.live Homepage" width=""></p>
</details>

### Admin Page
<details>
  <summary style="display: block;"></summary>
  <p align="center"><img src="./image/HypezAdmin.png" alt="Hypez.live Homepage" width=""></p>
</details>

## 🛠️ Local Development Setup

To run Hypez on your local machine, please follow the steps below:

### Prerequisites
* **Node.js** (version 18 or higher)
* **PostgreSQL** Database
* **Redis** (used for rate limiting and queue operations)

### Steps to Run
1. **Clone the repository:**
   ```bash
   git clone https://github.com/luluwux/hypez.live.git
   cd hypez.live
   ```
2. **Install dependencies:**
   ```bash
   npm install
   ```
3. **Configure Environment Variables:**
   Create `.env` files in root and inside `/apps` directories based on the `.env.example` templates.
4. **Setup Database Schema:**
   Apply database push and default seeds:
   ```bash
   npm run db:push
   npm run db:seed
   ```
5. **Run the development servers:**
   ```bash
   npm run dev
   ```

For detailed contributing guidelines, please check [CONTRIBUTING.md](./CONTRIBUTING.md).

## 🌟 Support

If you found this project helpful, please consider leaving a 🌟 star. Thank you!

<p align="center"\>
</p\>

- [My Discord Profile](https://discord.com/users/852103749228036136)

- If you find any errors, you can contact luppux
<br> </br>
<p align="center">
  <a href="https://discord.gg/luppux" target="_blank">
    <img src="https://api.weblutions.com/discord/invite/luppux/" alt="Discord Banner">
  </a>
</p>
<br> </br>
